...
nodes.push({"id": words1[0], "reflexive": true, "colour": "#007FFF", "radius": 70});

for (var i = 1; i < words1.length; i++) {
	nodes.push({"id": words1[i], "reflexive": true, "colour": "#AAA", "radius": 40});
}

nodes.push({"id": words2[0], "reflexive": true, "colour": "#007FFF", "radius": 70});
...
var index_s = -1, index_t = -1;

/*for (var i = 0; i < nodes.length; i++) {
	
	if (nodes[i].id === sources1[i]) {
		index_s = i;
		break;
	}
	
	if (nodes[i].id === targets1[i]) {
		index_t = i;
		break;
	}
	
	links.push({"source": nodes[index_s], "target": nodes[index_t], "left": false, "right": false, "colour": "#000", "length": 250});
}*/

for (var j = words1.length, k = j + 1; k < nodes.length; k++) {
...
function mousedown() {
  ...
  /*var point = d3.mouse(this),
      node = {"id": "", "reflexive": false, "colour": "#AAA", "radius": 40};
  node.x = point[0];
  node.y = point[1];
  nodes.push(node);*/
